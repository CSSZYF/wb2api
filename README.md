...
| **对话测试**（账号行内「测试」按钮） | 选模型 + 一条消息，用该账号真实发一次请求，回显回复摘要（前 200 字）、耗时与失败原因。**仅诊断**：不计入账号成功/失败统计，不触发冷却与熔断；**成功时**顺手解除该 (账号, 模型) 的模型级冷却（6004 限流豁免 / 11102「该后端无此模型」负缓存提前失效，不必等 TTL 到期），失败则原样不动 |
