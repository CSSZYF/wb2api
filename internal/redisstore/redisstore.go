...
type Upstash struct {
	...
	// 关 done 之后才跑到检查点会被误丢，close_test.go 稳定复现）。
	submitMu sync.Mutex
...
func (u *Upstash) goWrite(fn func()) {
	...
	go func() {
		defer u.wg.Done()
		// 只阻塞抢写槽，不检查 done：此处若 select done，已阻塞排队的写会在
		// close(done) 唤醒时全部走丢弃分支（实测 100%），「Close 前提交的写
		//（含排队中）必然执行」的契约被内部检查破坏——close_test.go 的排空
		// 用例因此间歇失败。丢弃语义已由提交点（submitMu 下的 done 检查）
		// 唯一承担；此处提交已冻结在 wg 中，Close 的 wg.Wait 必然等到它执行完。
		u.sem <- struct{}{}
		defer func() { <-u.sem }()
		fn()
	}()
