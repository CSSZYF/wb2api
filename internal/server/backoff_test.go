...
// TestBackoffAfterBounds 轮转退避的次数与抖动界（P0-2 验收点）：
...
func TestSleepCtxCancel(t *testing.T) {
	...
	// 已取消的 ctx 即便 d<=0 也返回 false（调用方「false 即终止轮转」语义单一）。
	cctx, ccancel := context.WithCancel(context.Background())
	ccancel()
	if sleepCtx(cctx, 0) {
		t.Fatal("cancelled ctx with d<=0 must return false")
	}
}

// TestRotateBackoffZeroBaseNoWait 测试态零退避：rotateBackoff 直接放行（ctx 未取消）。
func TestRotateBackoffZeroBaseNoWait(t *testing.T) {
	withRotateBackoff(t, 0)
	start := time.Now()
	if !rotateBackoff(3, context.Background()) {
		t.Fatal("zero base must pass without waiting")
	}
	if elapsed := time.Since(start); elapsed > 100*time.Millisecond {
		t.Fatalf("zero base must not wait, elapsed=%v", elapsed)
	}
}
