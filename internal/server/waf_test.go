...
// TestChatWaf403SoftCoolsWithoutDisable 端到端验收（P0-1）：
// WAF 403（空 body，非业务信封）→ 账号软冷却（CoolSoft，基数取
// soft_rate 基数；本测试注入 60s 以固定抖动界 [45s,75s]），
// **不 Disable**、不喂熔断（ErrServer 才喂 NoteError）。
...
func TestChatWaf403SoftCoolsWithoutDisable(t *testing.T) {
	...
	if d := time.Duration(st.CoolRemaining) * time.Second; d < 45*time.Second || d > 75*time.Second {
		t.Errorf("WAF cool_remaining=%v want in [45s,75s] (soft_rate base 60s ± 25%%)", d)
	}
...
// TestChatWaf403HtmlBodySoftCools HTML 拦截页形态（网关 WAF 页面）同 P0-1 分类。
...
// TestChatWafPassesThroughUpstreamBody 错误透传语义回归：WAF 403 的 message 仍
// 透传上游原文（排障必需）；空体（WAF 常见形态）时保留可读分类文案兜底。
func TestChatWafPassesThroughUpstreamBody(t *testing.T) {
	...
	if !strings.Contains(rec.Body.String(), "Forbidden: request blocked by WAF") {
		t.Errorf("503 message should carry upstream 原文: %s", rec.Body)
	}
}
...
// TestRotateBackoffRealDelay 轮转退避真实生效（P0-2 验收点）：首个号 WAF 403 后，
// 换 good 号前有可观测的退避间隔（测试态 base=0 被恢复为短基数 30ms，
// 抖动界 [22.5ms, 37.5ms]，断言总耗时 ≥ 22ms 且不超界太多）。
