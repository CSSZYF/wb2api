// wafip.go WAF IP 级拦截 fail-fast 状态机（WAF 403 修复任务）。
//
// 背景：WAF 403 拦的是网关出口 IP 而非账号——多个账号在极短时间内接连全 403。
// 既有 ErrWafBlock 账号级软冷却（soft_rate 基数起的有界退避）在 IP 级拦截时
// 不够：轮转会把一次客户端请求放大 MaxRotate 倍，同一出口 IP 继续打上游只会
// 加重风控。
//
...
// 被拦（同一出口 IP）已是 IP 级证据（实测 3 号 1s 全拦，阈值 2 更早止损，
// 少放大一次轮转）。单账号池时该阈值永不达到（只有一个 UID），行为不劣化。
...
func (g *wafIPGate) noteWaf(uid string) bool {
	...
	if len(g.hits) >= wafIPThreshold {
		...
		log.Printf("WARN: [server] waf ip-level block: %d accounts hit waf 403 within %s, rotate fail-fast until %s",
