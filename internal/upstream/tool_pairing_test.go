...
// TestCleanupOrphanToolCallsSymmetric 部分回结果（批 [c1,c2] 只回 c1）→ 按 keepCalls
// 对称裁剪：调用侧只留 c1（c2 被剔），结果侧只在配对时才留。旧行为「整批删或整批留」
// 会留下无主结果（无 tool_calls 的 assistant + 孤儿 tool），上游判 11148 报废会话。
func TestCleanupOrphanToolCallsSymmetric(t *testing.T) {
	messages := []any{
	...
	// 关键不变式：出站任何 tool 结果都必须有对应 tool_call，任何 tool_call 都必须有结果。
	assertPairingSymmetric(t, out)
}

// assertPairingSymmetric 断言出站消息两侧配对对称：每个 tool_call id 都有结果，
// 每个 tool 结果的 id 都有调用（否则上游判 11148 tool calls and tool results do not match）。
...
// TestCleanupOrphanOutOfOrderToolBeforeResult 乱序：tool 结果出现在 assistant tool_call
// 之前（不按顺序但 id 齐全）→ 仍保留（按 id 集合配对，与顺序无关）。
...
// <image_resize_notice> 作为 developer 消息插在并行 tool 结果中间，上游判 11148
// tool_call_sequence_broken。修复后结果必须连续、插入物后移、内容不变。
func TestRepackToolResultBlocksInsertedNotice(t *testing.T) {
	...
	// 只调顺序不改内容：developer 消息仍是原 map 实例。
...
// TestRepackToolResultBlocksNextGroupHeadNotSwallowed 回归：下一组 assistant.tool_calls
// 紧跟上一组结果时，绝不能被上一组的收集循环当「插入物」吞掉——否则它自己那批结果
// 永远得不到重排，上游照旧判 11148。
...
// TestRepackThenCleanupRealShape 集成（真实会话卡死形态）：多 tool 并行 + 图片 resize
// notice 插在结果中间，经 repack + cleanup 后出站载荷零违规——结果连续、配对对称。
// 走 PrepareBodyOpt 生产转换路径，而非只调内部函数。
